  ...
  run: async(client, message, args) => {
    ...
    if (language === "pt-BR") { // PT-BR
      ...
      if (Number(args[0]) < 0) {
        ...
        .setDescription(`> **${e.no} Infelizmente somente é possivel sacar valores positivos! Use por exemplo \`${prefix}withdraw 50000\`!**`)
        .setColor('WHITE')
    ...
    if (!language || language === "en") { // EN
      ...
      if (Number(args[0]) < 0) {
        const calculatorError2 = new MessageEmbed()
        .setDescription(`> **${e.no} Unfortunately it is only possible to draw positive values! Use for example \`${prefix}deposit 50000\`!**`)
        .setColor('WHITE')
