  ...
  run: async(client, message, args) => {
    ...
    if (language === "pt-BR") { // PT-BR
      ...
      if (Number(args[0]) < 0) {
        ...
        .setDescription(`> **${e.no} Infelizmente somente é possivel depositar valores positivos! Use por exemplo \`${prefix}deposit 50000\`!**`)
        .setColor('WHITE')
        .setFooter('Hey, Use apenas números positivos aqui!')
    ...
    if (!language || language === "en") { // EN
      ...
      if (Number(args[0]) < 0) {
        const calculatorError2 = new MessageEmbed()
        .setDescription(`> **${e.no} Unfortunately it is only possible to deposit positive values! Use for example \`${prefix}deposit 50000\`!**`)
        .setColor('WHITE')
        .setFooter('Hey, only use positive numbers here!')
        return message.reply({ embeds: [calculatorError2] })
      }
