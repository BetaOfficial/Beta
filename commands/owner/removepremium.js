    ...
    run: async (client, message, args) => {
        ...
        if (language === "pt-BR") { // PT-BR
            ...
            if (db.get(`premium_${user.id}`) === null || db.get(`premium_${user.id}`) === false) {
                ...
                let embed = new Discord.MessageEmbed()
                .setColor("WHITE")
                .setDescription(`> **${e.yes} ${user.tag} agora não está mais na minha lista de usuários premium!**`);
                message.reply({ embeds: [embed] })
        ...
        if (!language || language === "en") { // EN
            ...
            if (db.get(`premium_${user.id}`) === null || db.get(`premium_${user.id}`) === false) {
                ...
                let embed = new Discord.MessageEmbed()
                .setColor("WHITE")
                .setDescription(`> **${e.yes} ${user.tag} is now no longer on my premium users list!**`);
                message.reply({ embeds: [embed] })
                db.set(`premium_${user.id}`, false)
