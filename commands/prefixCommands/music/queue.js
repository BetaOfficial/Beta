    ...
    run: async (client, message, args) => {
        ...
        if (language === "pt-BR") { // PT-BR
            ...
            try {
                ...
                const embed = new MessageEmbed()
                    .setTitle(`<:yes:930170194784043048> Server Queue \`(${convertTime(queue.duration)})\``)
                    .setColor('WHITE')

                const tracks = queue.slice(0, 10)

                if (queue.current) embed.addField(`**Tocando agora:**`, `**[${queue.current.title}](${queue.current.uri}) \`(${convertTime(queue.current.duration)})\`**`)
                if (!tracks.length) embed.setDescription(`**Não há músicas na fila!**`)
                else embed.setDescription(
                    tracks.map((t, i) => {
                        return `**${i + 1} - [${t.title}](${t.uri}) \`(${convertTime(t.duration)})\`**`
                    })
                    .join('\n')
                )
        ...
        if (!language || language === "en") { // EN
            ...
            try {
                ...
                if (queue.current) embed.addField(`**Playing now:**`, `**[${queue.current.title}](${queue.current.uri}) \`(${convertTime(queue.current.duration)})\`**`)
