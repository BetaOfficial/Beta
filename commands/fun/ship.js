    ...
    run: async (client, message, args) => {
        ...
        if (language === "pt-BR") { // PT-BR
            if(user && user.id === message.author.id) {
                ...
                .setDescription(`**${e.no} Você não pode mencionar a sí mesmo duas vezes!**`)
                .setColor("WHITE")
                return message.reply({ embeds: [embed] })
            }
            if (!user) {
                let embed = new Discord.MessageEmbed()
                .setDescription(`**${e.no} Você precisa marcar alguém!**`)
                .setColor("WHITE")
                return message.reply({ embeds: [embed] })
            }
        ...
        if (!language || language === "en") { // EN
            if(user && user.id === message.author.id) {
                let embed = new Discord.MessageEmbed()
                .setDescription(`**${e.no} You can't mention yourself twice!**`)
                .setColor("WHITE")
                return message.reply({ embeds: [embed] })
            }
            if (!user) {
                let embed = new Discord.MessageEmbed()
                .setDescription(`**${e.no} You need to tag someone!**`)
                .setColor("WHITE")
                return message.reply({ embeds: [embed] })
            }
